...
contract MoonwellSupplyIntegrationTest is BaseIntegrationTest {
    ...
    ///      time). With no strategy bound the vault has no Lane A live-NAV
    ///      term, so `redemptionsLocked()` forces all mid-proposal exits
    ///      through the async redeem queue (Lane B), settling at the realized
    ///      frozen price.
