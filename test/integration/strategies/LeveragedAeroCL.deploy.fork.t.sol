...
contract LeveragedAeroCLDeployFork is LeveragedAeroForkBase {
    ...
    function test_deploy_riskParamsStored() public {
        ...
        assertEq(strategy.s().targetLtvBps, TARGET_LTV_BPS, "targetLtvBps");
        assertEq(strategy.s().maxLtvBps, MAX_LTV_BPS, "maxLtvBps");
        assertEq(strategy.s().minHealthBps, MIN_HEALTH_BPS, "minHealthBps");
        assertEq(strategy.s().maxSlippageBps, MAX_SLIPPAGE_BPS, "maxSlippageBps");
    }
    ...
    function test_deploy_usdcCollateralFactorRead() public {
        ...
        uint16 cfBps = strategy.s().usdcCollateralFactorBps;
    ...
    function test_deploy_addressesWired() public {
        ...
        assertEq(strategy.s().usdc, BaseAddresses.USDC, "usdc");
        assertEq(strategy.s().mUsdc, BaseAddresses.MOONWELL_MUSDC, "mUsdc");
        assertEq(strategy.s().mCbBTC, BaseAddresses.MOONWELL_MCBBTC, "mCbBTC");
        assertEq(strategy.s().mWeth, BaseAddresses.MOONWELL_MWETH, "mWeth");
        assertEq(strategy.s().pool, BaseAddresses.CBBTC_WETH_POOL, "pool");
        assertEq(strategy.s().gauge, BaseAddresses.CBBTC_WETH_GAUGE, "gauge");
        assertEq(strategy.s().npm, BaseAddresses.SLIPSTREAM_NPM, "npm");
        assertEq(strategy.s().tickSpacing, BaseAddresses.CBBTC_WETH_TICK_SPACING, "tickSpacing");
    }
    ...
    function test_deploy_feeParamsWired() public {
        ...
        assertEq(strategy.s().managementFeeBps, MGMT_FEE_BPS, "managementFeeBps");
        assertEq(strategy.s().performanceFeeBps, PERF_FEE_BPS, "performanceFeeBps");
        assertEq(strategy.s().feeRecipient, feeRecipient, "feeRecipient");
    }
    ...
    function test_deploy_tokenIdZero() public {
        ...
        assertEq(strategy.s().tokenId, 0, "tokenId should be 0 pre-deploy");
    }
    ...
    function test_execute_opensLeveredPositionWithinBounds() public {
        ...
        assertNotEq(strategy.s().posTickLower, strategy.s().posTickUpper, "ticks equal");
        ...
        uint256 cfBps = uint256(strategy.s().usdcCollateralFactorBps);
    ...
    function test_settle_returnsPrincipalAndClears() public {
        ...
        assertGt(strategy.s().tokenId, 0, "tokenId should be non-zero after execute");
        ...
        assertEq(strategy.s().tokenId, 0, "tokenId not cleared after settle");
    ...
    function test_settle_coversShortfallUnderIL() public {
        ...
        assertEq(strategy.s().tokenId, 0, "tokenId not cleared after shoved settle");
    }
