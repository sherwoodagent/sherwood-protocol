...
/// @dev Vault whose `strategyMint` can be toggled to revert (models a PAUSED vault / de-whitelisted
///      feeRecipient — both make the fee-share mint revert). `strategyBurn` always succeeds, mirroring
///      the real vault (strategyBurn is deliberately NOT `whenNotPaused`, so exits survive a pause).
