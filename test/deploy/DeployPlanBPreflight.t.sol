...
contract DeployPlanBPreflightTest is Test {
    ...
    ///      script's own constant so the two cannot drift. The default now sits
    ///      ON the ledger floor, so the two move together or not at all.
    function test_deploy_seatsTheShippedHaircut() public {
        assertEq(script.DEFAULT_WOOD_HAIRCUT_BPS(), 5_000, "the shipped haircut equals the ledger floor (5,000)");
