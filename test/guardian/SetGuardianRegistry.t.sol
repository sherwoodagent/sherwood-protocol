...
contract SetGuardianRegistryTest is Test {
    ...
    function test_setGuardianRegistry_isAbsentFromGovernorAndFactory() public {
        ...
        // Control: raw calls to these proxies do reach a selector that exists,
        // so the two failures above are the missing setter and not a proxy that
        // rejects every raw call.
        bytes memory present = abi.encodeWithSignature("guardianRegistry()");
        (bool govControlOk,) = address(governor).call(present);
        assertTrue(govControlOk, "control: a selector the governor has must answer");
        (bool factoryControlOk,) = address(factory).call(present);
        assertTrue(factoryControlOk, "control: a selector the factory has must answer");
    }
}
