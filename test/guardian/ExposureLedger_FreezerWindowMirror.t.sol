...
contract ExposureLedgerFreezerWindowMirrorTest is ExposureLedgerTest {
    ...
    ///         inverted.
    function test_bypassSequence_revertsAtTheRewire() public {
    ...
    function test_bypassSequence_controlWithAFittingGameSucceeds() public {
    ...
    function test_freezerWindowAboveTheLedgers_reverts() public {
    ...
    function test_equalOrShorterFreezerWindow_passes() public {
    ...
    function test_zeroFreezer_passesWithoutAWindowRead() public {
        MockCoverageFreezer game = _game(W);
        vm.startPrank(owner);
        ledger.setCoverageFreezer(address(game));
    ...
    function test_codelessFreezer_failsClosed() public {
    ...
    function test_muteFreezer_failsClosed() public {
    ...
    function test_shortReturndataFreezer_failsClosed() public {
    ...
    function test_longReturndataFreezer_failsClosed() public {
    ...
    function test_revertingFreezer_failsClosed() public {
        MockCoverageFreezer game = _game(W);
        game.setMuted(true);
        vm.prank(owner);
        vm.expectRevert(IExposureLedger.CoverageFreezerUnreadable.selector);
    ...
    function test_setChallengeWindow_unreadableWiredFreezer_failsClosedButIsRecoverable() public {
        MockCoverageFreezer game = _game(W);
        vm.startPrank(owner);
        ledger.setCoverageFreezer(address(game));
        game.setMuted(true);
    ...
    function test_setChallengeWindow_raiseSkipsAMuteFreezerEvenUnderALiveFreeze() public {
        MockCoverageFreezer game = _game(W);
        vm.prank(owner);
        ledger.setCoverageFreezer(address(game));
    ...
    ///         equal.
    function test_setChallengeWindow_cannotShrinkBelowTheWiredGame() public {
        MockCoverageFreezer game = _game(W);
        vm.startPrank(owner);
        ledger.setCoverageFreezer(address(game));
        vm.expectRevert(IExposureLedger.InvalidParameter.selector);
    ...
    function test_loweringSequence_gameFirstThenLedger_succeeds() public {
    ...
    function test_raisingSequence_ledgerFirstThenGame_succeeds() public {
        ChallengeGame game = new ChallengeGame(owner, makeAddr("wood"), address(ledger), makeAddr("tiers"));
        vm.startPrank(owner);
        ledger.setCoverageFreezer(address(game));
