...
contract ClockAndBasisTest is RegistryTestHarness {
    ...
    ///      dead proposal.
    function test_cancelReviewSucceedsInsideThePauseDeferredSpan() public {
        _stakeFullCohort();
    ...
    function test_cancelStillRefusedOnceTheEffectiveClockPasses() public {
        _stakeFullCohort();
        uint256 voteEnd = vm.getBlockTimestamp() + 1 hours;
        uint256 reviewEnd = voteEnd + REVIEW_PERIOD;
        _registerReview(PROPOSAL_ID, voteEnd, reviewEnd);
    ...
    ///      ordinary case.
    function test_noPauseLeavesTheCancelWindowUnchanged() public {
        _stakeFullCohort();
        uint256 voteEnd = vm.getBlockTimestamp() + 1 hours;
        uint256 reviewEnd = voteEnd + REVIEW_PERIOD;
        _registerReview(PROPOSAL_ID, voteEnd, reviewEnd);
    ...
    function test_reviewRegisteredMidPauseIsNotCreditedTheWholeOutage() public {
        _stakeFullCohort();
    ...
    ///      reading anything.
    function test_readsDuringTheSamePauseDoNotUnderflow() public {
        _stakeFullCohort();
    ...
    function test_pauseAfterRegistrationStillDefersInFull() public {
        _stakeFullCohort();
        uint256 voteEnd = vm.getBlockTimestamp() + 1 hours;
        uint256 reviewEnd = voteEnd + REVIEW_PERIOD;
        _registerReview(PROPOSAL_ID, voteEnd, reviewEnd);
