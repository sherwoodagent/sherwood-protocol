...
contract RegistryDemoteKeepsCalleeStandingTest is Test {
    ...
    /// @notice A grant re-opens the callee axis on both axes, and announces it
    ///         on the callee event an indexer follows.
    function test_grantAnnouncesTheReopenedCalleeAxis() public {
        tierRegistry.setCallable(address(strategy), false);
        vm.expectEmit(true, false, false, true, address(tierRegistry));
        emit TierRegistry.CalleeAllowedSet(address(strategy), true);
        tierRegistry.setAdapterAllowed(address(strategy), true);

        address clone = _certifiedClassClone();
        bytes32 cch = tierRegistry.cloneCodehashOf(address(strategy));
        tierRegistry.setClassCallable(address(strategy), false);
        vm.expectEmit(true, true, false, true, address(tierRegistry));
        emit TierRegistry.ClassCalleeAllowedSet(address(strategy), cch, true);
        tierRegistry.setClassAllowed(address(strategy), true);
        assertTrue(tierRegistry.isCallableTarget(clone), "the class callee axis is open again");
    }
