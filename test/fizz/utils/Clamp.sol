    ...
    function clampLt(int256 a, int256 b) internal returns (int256) {
        if (!(a < b)) {
            if (b == type(int256).min) {
                Logger.logString(
                    "clampLt cannot clamp value a to be less than int256.min. Check your inputs/assumptions."
                );
                assert(false);
            }
            int256 value = b - 1;
    ...
    function clampGt(int256 a, int256 b) internal returns (int256) {
        if (!(a > b)) {
            if (b == type(int256).max) {
                Logger.logString(
                    "clampGt cannot clamp value a to be larger than int256.max. Check your inputs/assumptions."
                );
                assert(false);
            }
            int256 value = b + 1;
