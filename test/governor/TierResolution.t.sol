...
contract TierResolutionTest is Test {
    ...
    function test_allCertifiedTier0CallsYieldTier0Coverage() public {
        ...
        _certifyNow(address(usdc), usdc.approve.selector, 0, 50); // the settle call
    ...
    function test_mixedTier0AndTier1CoverageIsSumNotMax() public {
        _wireTierRegistry();
        _certifyNow(address(mockAdapter), mockAdapter.approve.selector, 0, 50);
        _certifyNow(address(mockAdapter), mockAdapter.transfer.selector, 1, 200);
        _certifyNow(address(usdc), usdc.approve.selector, 0, 50); // the settle call
    ...
    function test_executeRevertsWhenCoverageRegressedAtSameTier() public {
        ...
        // The replacement: same tier 0, 10x the extractable bound.
        _certifyNow(address(mockAdapter), mockAdapter.approve.selector, 0, 500);
        (uint8 liveTier,) = tierRegistry.tierOf(address(mockAdapter), mockAdapter.approve.selector);
