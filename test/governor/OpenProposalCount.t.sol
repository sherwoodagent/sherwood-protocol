...
contract OpenProposalCountTest is Test {
    ...
    function test_unexecutedPastExecuteBy_resolveAllowsPropose() public {
        ...
        // `resolveProposalState`'s lazy-resolution `_decOpen` also arms the
        // settle cooldown (PR #359 review #1, see
        // `test_resolveProposalState_armsCooldownLikeVeto`), so the next
        // propose must clear it first — same pattern used there.
