...
#### Scenario: Rate saturated at the arithmetic cap, live envelope ignored
- **WHEN** the slasher passes a non-zero per-approver rate above `10_000`, or the owner moves `minSlashBps` / `maxSlashBps` between the caller's pin and the call
- **THEN** only the `10_000` saturation applies — the live slots move neither the floor nor the ceiling of an already-decided burn
