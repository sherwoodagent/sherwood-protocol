...
### Requirement: Verdict proceeds burn, with no payee to name
`slashVerdict(caseKey, openedAt, approvers, slashBpsPer)` SHALL take no recipient and no rate for one: every wei it recovers SHALL be burned. The burn goes through `_burnWood`, which parks the amount in `_pendingBurn` for a permissionless `flushBurn` retry if the WOOD transfer reverts or returns false, so a hostile token cannot brick a conviction whose accounting already landed. `VerdictSlashBurned(caseKey, burned)` SHALL be emitted with `burned` equal to the returned total. When nothing was recovered across all approvers, `0` is returned and nothing is burned. The absence of a sink is the security property, not an omission: a slash-funded payout is a pot a prosecutor can fill for itself by staking, approving the proposal it is about to accuse, and collecting a share sized by its own punishment. The challenger is instead paid from the convicted PROPOSER's forfeited bond, the one pot a prosecutor cannot self-fund, and that payment is made by `ProposerBondEscrow`, never here.

#### Scenario: Successful verdict burns the whole recovery
- **WHEN** `slashVerdict` recovers WOOD
- **THEN** the entire total is burned and `VerdictSlashBurned(caseKey, burned)` reports it

#### Scenario: Hostile token cannot brick a landed conviction
- **WHEN** the WOOD transfer to the burn address reverts or returns false
- **THEN** the stake accounting still lands and the amount is parked in `_pendingBurn` for a permissionless `flushBurn`

#### Scenario: No caller-named destination exists
- **WHEN** a compromised slasher tries to direct verdict proceeds anywhere
