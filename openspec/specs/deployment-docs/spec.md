...
#### Scenario: Spot diverging from the feed refused on the finishing run
- **WHEN** the WOOD/WETH spot read on the run that mints Plan B diverges more than 2x from the already-answering WOOD feed's own price
- **THEN** the run refuses before Plan B is minted, naming the divergence; the first run, whose cap is discarded at the stage gate, is unaffected
