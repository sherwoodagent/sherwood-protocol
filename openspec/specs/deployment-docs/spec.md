...
The broadcaster SHALL equal the book's `DEPLOYER`, and the script reverts "broadcaster != DEPLOYER in the address book" otherwise. There is no skip flag: handoff is a property of Mainnet posture. A Fork book carrying `OWNER_MULTISIG` is REFUSED ("Fork posture never hands off: remove OWNER_MULTISIG from this chain's address book"); on Mainnet `OWNER_MULTISIG` is REQUIRED and MUST be a contract (Safe), not an EOA.
...
- The cap constant is `WOOD_PRICE_CAP_X8`, RENAMED from `WOOD_PRICE_HAIRCUT_X8` because the number's meaning inverted: it is a ceiling on manipulation, never served as a price, and SHALL be seeded **ABOVE** market. The ceremony BOUNDS it at deploy time to `[1.25x, 2x]` the pool spot it derives from the live WOOD/WETH pair — below 1.25x the cap binds permanently and pins every bond, above 2x it stops bounding manipulation — and refuses the run outside that band. The old "≤ 30-day low" instruction is now exactly backwards.
...
- **THEN** pre-flight 10 does not run at all, because the Fork posture performs no handoff — the waiver is derived from posture rather than set by an operator, so no key exists that could waive it on mainnet
