...
### Requirement: Compensation pay-through to queued exiters
The queue SHALL pay compensation-escrow cases through to redeem-request owners whose shares sat in queue custody at the case snapshot. `claimCompensation(caseId, requestIds)` SHALL be permissionless with these properties: the escrow address is resolved from governance (`factory.compensationEscrow()`), never caller-supplied (`CompensationEscrowNotSet` when unset); the first call for a case pulls the queue's whole claim, verifying the case binds this vault (`NotCompensationCase`), measuring the received amount by balance delta (a transfer-nothing escrow distributes nothing; `NothingToClaim` on zero votes or zero receipt), and pinning the payout token at pull time so a later escrow re-point or token flip cannot change the payout unit; case bookkeeping is keyed by (escrow, caseId) so a case funded by one escrow is never paid from another's accounting. Each supplied redeem request whose custody interval `[queuedAt, closedAt)` covers the snapshot (and not already paid for the case) SHALL receive `mulDiv(caseTotal, requestShares, queueVotesAtSnapshot)` paid to the request's own owner; ineligible or already-paid ids SHALL be skipped (with `CompensationSkipped`), not reverted, while out-of-range ids revert the batch. An empty id array SHALL revert `NoRequestsSupplied`, and a call that pays zero total SHALL revert `NoEligibleRequests`, rolling back the pull so proceeds never park idle in the queue.

#### Scenario: Governance re-point cannot redirect a pulled case
- **WHEN** a case was pulled under escrow E1 and governance later re-points the factory to escrow E2
- **THEN** E1's case bookkeeping (total, votes, pinned token) is untouched by E2, and payouts for that case continue in the token pinned at pull time

#### Scenario: Front-run claim does not grief a keeper batch
- **WHEN** one id in a keeper's batch was already paid for the case
- **THEN** that id is skipped and counted in `skipped` while the rest of the batch pays out

#### Scenario: Pull-and-park is impossible
- **WHEN** a call would pull a case's proceeds but distribute nothing (all ids ineligible or all shares flooring to zero)
- **THEN** the whole transaction reverts `NoEligibleRequests`, leaving the escrow case redeemable

#### Scenario: Payout destination is never caller-chosen
- **WHEN** any caller invokes `claimCompensation`
- **THEN** every payout goes to the named requests' own owners; the caller selects only which requests are processed
