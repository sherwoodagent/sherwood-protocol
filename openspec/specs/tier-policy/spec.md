...
Demotion SHALL delete the tier config (the key reverts to the tier-2 default), start the bond release timelock exactly once (`releasableAt = block.timestamp + bondReleaseDelay`, emitting `SubmitterBondReleaseStarted`, only if a bond exists and is not already releasing), and emit `TierDemoted`. Three callers reach it:
...
- **THEN** the config is deleted, the bond release timelock starts, and `TierDemoted` is emitted
