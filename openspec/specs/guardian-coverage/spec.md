...
`setChallengeWindow` SHALL reject zero, SHALL enforce the scan bound, and — when a `coverageFreezer` (the challenge game) is wired and the new window is BELOW the current one — SHALL reject a window below the game's own, since the game's `file` deadline is keyed off its window and the ledger's `retireApproval` gate off this one. A raise SHALL NOT read the game: it cannot violate `game.challengeWindow <= ledger.challengeWindow`, so the safe direction stays available even when the wired freezer no longer answers and a live freeze blocks unwiring. The read of the game's window SHALL fail closed: a non-zero freezer that does not answer `challengeWindow()` with exactly one word — codeless, reverting, or returning any other length — makes the call revert `CoverageFreezerUnreadable` rather than dropping the bound. `setCoverageFreezer` SHALL mirror the same bound — a non-zero freezer whose `challengeWindow()` exceeds the ledger's SHALL be refused `InvalidParameter`, and one that cannot answer it SHALL be refused `CoverageFreezerUnreadable` — so that neither wiring a longer-windowed game nor `unwire → shrink → re-wire` can reach `game.challengeWindow > ledger.challengeWindow`. A codeless freezer SHALL be refused by both: `SyndicateGovernor.reclaimProposerBond` makes a typed `challengeWindow()` call on the wired freezer, which a codeless address would revert, and a pre-committed address could gain code later with no check firing (SHE-214). Zero SHALL remain the unwire switch and SHALL never be read. The window applies retroactively to already-booked buckets: shrinking it frees coverage early, growing it re-counts expired buckets.
...
- **WHEN** the owner calls `setCoverageFreezer` with a freezer whose `challengeWindow()` exceeds `ledger.challengeWindow`
- **THEN** the call reverts `InvalidParameter`
...
- **WHEN** a non-zero freezer does not answer `challengeWindow()` with exactly one word — codeless, reverting, or returning another length — at wiring, or later while `setChallengeWindow` lowers the window
- **THEN** the call reverts `CoverageFreezerUnreadable`

#### Scenario: Raising the window past a freezer that stopped answering
- **WHEN** the wired freezer no longer answers `challengeWindow()`, a freeze is live (so `setCoverageFreezer(0)` reverts `CoverageFrozen`), and the owner raises the challenge window
- **THEN** the raise succeeds without reading the game; lowering it still reverts `CoverageFreezerUnreadable`
