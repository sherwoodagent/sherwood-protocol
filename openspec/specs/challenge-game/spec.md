...
### Requirement: The challenge vote — electorate pinned at filing, accused excluded
`file` SHALL pin the challenge's electorate in the same call, as `votableStakeAtFiling = stakedWood.getPastTotalVotes(block.timestamp - 1)` less `stakedWood.getPastStake(accused_i, block.timestamp - 1)` for every accused approver, saturating at zero. The stamp SHALL be one second before the filing, never the filing instant: an sWOOD checkpoint is keyed on the second a stake changes and a same-key push overwrites, so reading the current timestamp would let stake planted in the filing block itself score in the numerator while the denominator missed it. When the difference is zero — the accused cohort is the entire staked guardian set — `file` SHALL revert `NoVotableStake` rather than accept a bond no electorate could ever adjudicate. `file` SHALL also record each accused approver in an O(1) membership map for the vote's own gate, written in the loop it already runs over the cohort, and SHALL revert `ZeroAddress` when `stakedWood` is unwired.

#### Scenario: Electorate measured one second before the filing
- **WHEN** a guardian stakes WOOD in the same block as a filing
- **THEN** that stake is in neither `votableStakeAtFiling` nor any ballot weight — both read `filedAt - 1`
...
### Requirement: Resolution — quorum settles, a closed window short of it fails
`resolve(challengeId)` SHALL be permissionless and choose nothing; the outcome is fixed by the tallies and the clock. On a `Filed` challenge it SHALL settle immediately when `votableStakeAtFiling != 0` and `convictWeight * 10_000 >= quorumBpsAtFiling * votableStakeAtFiling` — the tally is monotone, so a crossed quorum is already final and need not wait out the window. Otherwise it SHALL revert `DelayNotElapsed` before `filedAt + voteWindowAtFiling`, and at or after it SHALL fail the challenge. From any terminal status it SHALL revert `WrongStatus`. `filedAt + voteWindowAtFiling` is therefore the hard end of the accusation: it is the same value `file` books on the ledger as the freeze deadline, and no ballot can be cast from that instant on, so a filing short of quorum at that moment can only fail.

#### Scenario: Quorum settles without waiting for the window
- **WHEN** convict weight crosses `quorumBpsAtFiling` of `votableStakeAtFiling` on day two of a seven-day window
