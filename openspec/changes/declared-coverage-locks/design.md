...
`judge.ts` already derives a prospective coverage placement per proposal — the guardian's own free capacity, bounded by an operator-configured per-proposal ceiling (`withinCeiling`), surfaced as `selfCoverageFloorMaxCapital` — before deciding APPROVE/ABSTAIN/BLOCK. The declared lock is that placement, in WOOD. No new knob: the operator ceiling that already bounds concentration is the lock's upper bound.
