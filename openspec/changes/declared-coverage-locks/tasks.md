...
- [x] 7.5 (done in ~/code/sherwood/app, uncommitted there; app typecheck not run — node_modules absent) App: `sherwood/app/src/lib/selector-registry.ts` pins the approve-vote selector; the new `voteOnProposal` signature changes it — update the registry entry (and any decoder that renders the vote) so the app does not label the new call as unknown.
