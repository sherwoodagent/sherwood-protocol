...
2. `target == asset()`: a call is either one of the two metered transfers or allowance-shaped. `transfer(to, n)` and `transferFrom(vault, to, n)` are metered egress; `transferFrom` whose `from` word is not the vault reverts `TransferFromNotVault(from)`. EVERY other call on the asset is treated as allowance-shaped: it must carry at least 36 bytes (`MalformedAssetCall(selector)` otherwise) and its first argument word is recorded as a spender for rule 3, whatever the selector. No selector is enumerated: `approve`, `increaseAllowance`, Paxos's `increaseApproval` (the launch asset, USDG, has that one and not `increaseAllowance`), `approveAndCall`, any future grant shape, and harmless reads (`balanceOf(x)` records `x`, whose reset is a no-op) are all covered by the same rule. A selector the token does not have reverts in the token.
3. Every recorded spender is `forceApprove(spender, 0)`'d after the delegatecall returns, before the meters. A spender that did not pull loses the allowance; a spender that pulled has already been metered; an address that never held one is a no-op reset. No dedup: a double reset is a no-op.
...
`StrategyFactory.registerStrategy(strategy)` is callable by anyone, with no fee. It requires code and that the three `IStrategy` getters `vault()`, `proposer()`, `executed()` each answer one word; it records the address and its codehash and emits `StrategyRegistered`. `isRegisteredStrategy` is true only while the codehash still matches, so a code change after registration de-registers. That pin sees the code AT the address: a per-address delegatecall proxy keeps its registration AND any per-address certified tier across an implementation swap, because the proxy's own code never changes (measured: register an admin-swappable proxy, certify `execute` at tier 1, swap the implementation to one that reverts — still registered, still tier 1). Policy: the protocol certifies non-upgradeable contracts only, and the registry owner refuses a proxy at certification time (EIP-1967 implementation slot empty, no `DELEGATECALL` in the runtime code — a review-time check, not enforceable on-chain). Class clones are immune: an EIP-1167 clone targets a fixed template. `cloneAndInit` and its deterministic twin register the clone they mint. Registration does NOT require `vault() == anything`: a strategy may serve several vaults if its code allows.
...
### D10 — Settle batches have zero egress budget

`settleProposal` and `unstick` pass `maxNetOutflow = 0` to `executeGovernorBatch`: the settle batch may bring assets home (net inflow or zero) and reverts `MaxNetOutflowExceeded(netOutflow, 0)` on any net egress. The execute batch keeps `effectiveMaxCapital`. So the proposal's declared capital bounds the WHOLE lifecycle's egress, not each leg — previously execute and settle each carried `effectiveMaxCapital`, a 2× lifecycle bound the PR body did not state. Per-call caps still meter the gross a settlement call may move (a strategy re-pulling what it deployed and pushing it back nets to zero). The three shipped templates settle through `[strategy.settle()]`, which pushes assets IN; the CL deleverage loop runs inside the strategy. `finalizeEmergencySettle` is the one settle path that keeps `effectiveMaxCapital`: an owner-supplied unwind may need to fund a repay from the vault to free stuck collateral, and that path is guardian-reviewed and owner-bonded for exactly that.

### D11 — The StrategyFactory phase wires the registry, mandatorily

With rule 1 an unwired `TierRegistry.strategyFactory()` is no longer "everything prices tier 2": every `propose` reverts `StrategyNotRegistered` and every non-asset batch target is refused. `DeployStrategyFactory` therefore has no deferred path: it requires `TIER_REGISTRY` in the book and `owner() == deployer`, calls `setStrategyFactory`, and asserts the wiring landed. The phase runs BEFORE the multisig accepts TierRegistry ownership (Ownable2Step keeps `owner()` at the deployer until `acceptOwnership`). A TierRegistry redeploy re-seeds `setStrategyFactory` alongside the certification set.
