...
- **THEN** `_seedTierRegistry` REVERTS rather than skipping, which `test/deploy/DeployTierRegistrySeed.t.sol` pins — the ceremony can no longer ship an empty registry while looking clean
