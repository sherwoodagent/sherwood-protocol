...
- [ ] 3.2 Confirm no mock ledger used in reclaim paths lacks `coverageFreezer()` (all three covering suites — `GovernorCoverageGates`, `ChallengeEndToEnd`, `CoverageEndToEnd` — wire the real `ExposureLedger`, so this is expected to be a no-op; fix any straggler by returning `address(0)`).
