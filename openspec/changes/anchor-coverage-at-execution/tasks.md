...
- [ ] 1.1 Extract `_slashOne`'s sizing (StakedWood.sol:1348-1352) into an
...
- [ ] 1.2 Add external view `slashableStakeAt(address guardian, uint256 anchor)`
...
- [ ] 1.3 Declare the view in `src/interfaces/IStakedWood.sol`.
- [ ] 1.4 Verify `./script/check-layout-goldens.sh` passes with ZERO golden
      diffs (a diff here is a bug in this change, not a golden to update).

## 2. Governor — expose the anchor (design D3)

- [ ] 2.1 Append `uint256 executedAt` to `SyndicateGovernor.ProposalViewLite`
      (SyndicateGovernor.sol:909-915) and populate it in `getProposalView`
      from `proposal.executedAt` (0 until executed).
- [ ] 2.2 Mirror the field in `ILedgerGovernorMinimal.ProposalViewLite`
      (ExposureLedger.sol:26-32).
- [ ] 2.3 Add the invariant natspec at SyndicateGovernor.sol:382/:429 — the
...
- [ ] 3.1 Make `_slashableBondUsd` anchor-aware: anchored variant values the
      guardian at `swood.slashableStakeAt(g, anchor)` when `anchor != 0`,
      live `guardianStake(g)` when `anchor == 0`; same `× priceX8 / 1e8`.
- [ ] 3.2 Thread `pv.executedAt` as the anchor through the four
...
- [ ] 3.3 Natspec corrections (design D7): settle residue bound
...
- [ ] 4.1 Update mocks for the ABI/interface changes in the same commit as
...
- [ ] 4.2 StakedWood tests: `slashableStakeAt` equals `_slashOne`'s recovery
...
- [ ] 4.3 Ledger tests: post-execution top-up moves neither `allocatedUsd`,
      ...
      unchanged.
- [ ] 4.4 End-to-end (ChallengeGame or CoverageEndToEnd harness): accused
      ...
      prosecution").
- [ ] 4.5 Regression sweep: the existing downward-clamp and price-path tests
...
- [ ] 5.1 `forge test` full suite; `forge fmt` with a CI-matching forge;
      `./script/check-layout-goldens.sh` with zero diffs.
- [ ] 5.2 Serialize builds on this machine (single `via_ir` solc at a time;
