...
- [ ] 5.3 Promote residual allowance, residual balance, native imbalance, and non-vault beneficiary to critical once each has fired zero false positives across the collected set — HELD, now with the evidence the earlier hold lacked. The checks are reachable end-to-end (`spendersChecked: 2`, `tokensChecked: 5` on a real proposal, allowances genuinely zero) and show zero false positives across four vnet proposals. But also zero TRUE positives: nothing malicious has been in front of them outside unit fixtures, so there is no evidence they would catch anything. `SWAP_RECIPIENT_NOT_VAULT` is already critical.
