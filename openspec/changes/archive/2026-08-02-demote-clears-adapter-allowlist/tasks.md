...
- [x] 1.1 In `_demote` (src/TierRegistry.sol:270-280), after the bond
...
- [x] 1.2 Extend the `_demote` natspec: state that every demotion path
...
- [x] 1.3 Extend the allowlist-section natspec (`setAdapterAllowed` /
...
- [x] 2.1 `test_demote_clearsAdapterAllowlist`: certify + allowlist a
      target; owner `demote`; assert `isAdapterAllowed == false` and
      `AdapterAllowedSet(target, false)` emitted.
- [x] 2.2 `test_demoteByChallenge_clearsAdapterAllowlist`: certify +
...
- [x] 2.3 `test_poke_clearsAdapterAllowlist`: certify + allowlist;
      `vm.etch` a different bytecode onto the target; permissionless
      `poke` from a rando; assert cleared + event.
- [x] 2.4 `test_demote_neverAllowlisted_noAllowedSetEvent`: demote a
...
- [x] 2.5 `test_demoteOneSelector_clearsWholeAdapter_intended`: certify
...
- [x] 2.6 `test_recertify_doesNotRestoreAllowlist`: certify + allowlist;
      ...
      still false.
- [x] 2.7 `test_ownerReallowsAfterClear`: after a demotion-clear, owner
...
- [x] 3.1 Rewrite the "`_demote` does not touch the allowlist — and you
...
- [x] 3.2 Narrow the "Operational consequence" paragraph: `TierDemoted`
...
- [x] 3.3 Update the §4 intro ordering note: allowlist-off-first remains
...
- [x] 3.4 Re-verify every `src/TierRegistry.sol` line reference in the
      checklist against the post-change file (the doc pins line
      numbers; `_demote` grows by ~4 lines).

## 4. Verify

- [x] 4.1 `forge fmt` (with a forge matching CI) and `forge build`.
- [x] 4.2 Run the full `forge test` suite in the foreground (serialize
...
- [x] 4.3 `openspec validate --change demote-clears-adapter-allowlist`.
