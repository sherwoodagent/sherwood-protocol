...
- [x] 1. Identify true upstream pins: for each of the four deps, determine
  ...
  divergence honestly.
- [x] 2. Write `lib/VENDOR-MANIFEST.json` (repo, ref, sha, pruned paths,
  local modifications).
- [x] 3. Write `script/check-vendor-provenance.sh` (bash, shallow clone at
  pinned ref into a temp dir, diff -r with exclusions, named output, exit
  codes per spec). Make it runnable locally.
- [x] 4. Run it locally end-to-end; capture output for the PR body.
- [x] 5. Add `.github/workflows/provenance.yml` (path-filtered PR trigger +
  workflow_dispatch + monthly off-minute schedule). Validate YAML
  (python3 yaml.safe_load).
- [x] 6. Commit on `chore/dependency-provenance`, push, open PR
