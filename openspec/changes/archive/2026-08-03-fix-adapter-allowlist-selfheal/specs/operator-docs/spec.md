...
<!-- Change: fix-adapter-allowlist-selfheal (issue #137). The allowlist's new
     codehash binding changes what the checklist must teach: grants bind to
     the code the owner saw, an un-poked codehash drift no longer leaves the
     FUNDS PATH open (only stale storage/indexer state), and legitimate
     upgrades need an explicit re-grant. Full requirement block carried per
     MODIFIED-delta convention. -->
