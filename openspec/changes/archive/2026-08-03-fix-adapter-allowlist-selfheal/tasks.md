...
- [x] 1.2 Add an internal view helper
...
- [x] 1.3 In `setAdapterAllowed` (`src/TierRegistry.sol:365-368`): on the
...
- [x] 1.4 In `isAdapterAllowed` (`src/TierRegistry.sol:372-374`): return
...
- [x] 2.1 Update `test_poke_clearsAdapterAllowlist` (`:532-546`): the
...
- [x] 2.2 New: metamorphic swap self-heals without poke — grant a contract
      ...
      snapshot).
- [x] 2.3 New: uncertified allowlisted adapter is protected — grant a
      ...
      cleanup.
- [x] 2.4 New: zero-code semantics (design.md D3) — (a) grant a codeless
...
- [x] 2.5 New: `setAdapterAllowed(adapter, false)` still answers false
...
- [x] 3.1 New test: the issue's exact attack path dies in the guard —
...
- [x] 4.1 Update `docs/adapter-onboarding-checklist.md` per the
...
- [x] 5.1 `openspec validate fix-adapter-allowlist-selfheal --strict`
      passes.
- [x] 5.2 Full `forge test` green (respect the machine's build
...
- [x] 5.3 Confirm no golden churn: `git status` shows no
      `script/*.golden.json` changes (TierRegistry is not pinned;
      touching a golden means something went wrong).
- [x] 5.4 `forge fmt` with a CI-matching forge before pushing (CI checks
      `src/`, `test/`, `script/`).
- [x] 5.5 Sanity: repo-wide grep still shows `SyndicateVault.sol:667`
