...
- **New capability `tokenize-fund-strategy`**: `src/strategies/TokenizeFundStrategy.sol`, a `BaseStrategy` clone template:
  ...
  - `settle()` — return quote-asset proceeds and collected creator fees to the vault; declare any remaining fund-token custody through the `IStrategyDelivery` views (`hasUnvaluedResidue`) with a `sweep()` recovery path, per `docs/nav-residue-design.md`.
