...
The deploy ceremony SHALL gain `script/robinhood-mainnet/DeployTokenizeFundStrategy.s.sol`, runnable after the core stack, deploying the `TokenizeFundStrategy` template, the `SushiLaunchAdapter` singleton, and the `StonkLaunchAdapter` implementation, then approving the template on `StrategyFactory`.
...
- **THEN** the tokenize-fund script is skipped and no template approval for it is written
