...
- **THEN** the call reverts; the creator role is exercisable only through the owner — the sole exception is post-settlement `forwardToVault()`, which pays the vault and no caller
