...
New capability spec. `VeniceInferenceStrategy` predates the openspec tree;
this delta adds only the requirement introduced by this change (adapter
provenance for `aeroRouter` and `sVVV`). It is not a retrofit of the whole
contract.
