...
- [x] 0.1 PR #158 (`fix/issue-35-booking-anchor`) is MERGED. Re-verify its
      ...
      implementation.
- [x] 0.2 Issue #43 (`per-call-capital-declarations`) is MERGED. Diff its
...
- [x] 0.3 Rebase this change's branch onto post-#43 main; regenerate nothing
      yet (goldens are task 5, AFTER the struct edit).
- [x] 0.4 Re-list `broadcast/` — confirm still no chain-4663 entry (design
...
- [x] 1.1 Change `requireApproveQuorum` (src/ExposureLedger.sol:1298 on
      ...
      (haveUsd, needUsd);`.
- [x] 1.2 Rewrite the function's natspec: "eligibility floor" framing becomes
...
- [x] 1.3 Update `IExposureLedger.sol:91` with the return values + natspec.
      Confirm by grep that `SyndicateGovernor.sol` is the ONLY `src/` caller
      (WoodTwapOracle mentions the name in a comment only).
- [x] 1.4 Sweep `test/ExposureLedger.t.sol` and `test/CoverageEndToEnd.t.sol`
...
- [x] 2.1 Append `uint256 effectiveMaxCapital` as the LAST member of
...
- [x] 2.2 In `executeProposal`: capture `(coverageRaisedUsd,
      ...
      `proposal.maxCapital`.
- [x] 2.3 In `settleProposal`: pass the STORED `proposal.effectiveMaxCapital`
      to the settlement `executeGovernorBatch` instead of
      `proposal.maxCapital`.
- [x] 2.4 Add `event EffectiveMaxCapitalSet(uint256 indexed proposalId,
...
- [x] 2.5 Add view `getEffectiveMaxCapital(uint256 proposalId) returns
...
- [x] 3.1 At execute, scale the execute-leg caps: `effectiveCap_i =
      ...
      batch.
- [x] 3.2 At execute, scale the settlement caps by the SAME factor and
...
- [x] 3.3 In `settleProposal`, pass the PERSISTED scaled settlement caps — no
      recompute at settle.

## 4. Tests (the issue's four obligations + this change's edges)

- [x] 4.1 40% coverage -> executes with `effectiveMaxCapital` at 40% of
      declared; the vault's outflow meter reverts an attempt to move more
      (issue obligation 1).
- [x] 4.2 Zero covering approvers (empty set AND all-zero contributions) ->
      still reverts `InsufficientApproveCoverage`, proposal stays `Approved`
      (issue obligation 2, design D2).
- [x] 4.3 Surplus coverage -> `effectiveMaxCapital == maxCapital` exactly
      (issue obligation 3).
- [x] 4.4 Settlement uses the same effective ceiling as execute (issue
...
- [x] 4.5 Dust coverage floors `effectiveMaxCapital` to 0 -> execution
      proceeds, zero net outflow permitted, batch with any outflow reverts
      (design D5 fail-closed).
- [x] 4.6 Gate-skipped paths (`requiredCoverage == 0` / no ledger) store
      `effectiveMaxCapital == maxCapital`; `EffectiveMaxCapitalSet` emitted
      with zero USD figures.
- [x] 4.7 Post-#43: per-call caps scale by the same factor; `Σ effectiveCaps
...
- [x] 4.8 Update `test/GovernorCoverageGates.t.sol` shortfall cases: the old
...
- [x] 5.1 Regenerate `script/syndicate-governor-layout.golden.json` AFTER the
      struct edit (and after #43's own regeneration is in the base).
- [x] 5.2 REVIEW THE DIFF as its own task: append-only — the only changes are
...
- [x] 5.3 `./script/check-layout-goldens.sh` passes.

## 6. Validation

- [x] 6.1 Full non-integration suite green; run forge in the FOREGROUND with
      the repo's build-lock discipline (serialize on `pgrep -x forge`, never
      judge a build through a piped exit code).
- [x] 6.2 `forge fmt` with a CI-matching forge version.
- [x] 6.3 `openspec validate --changes --strict` clean.
- [ ] 6.4 Comment on issue #27 with the landed shape (use `--body-file` —
      never inline backticks in `gh` bodies). NOT done by this pass — the PR
      itself (referencing `Fixes #27`) carries the landed-shape summary; a
      separate issue comment was outside this pass's explicit instructions.
