...
- [x] 1. Recon: read `script/DeployPlanB.s.sol` AS IT IS ON THIS BRANCH
...
- [x] 2. Implement the seat + both asserts per the spec delta, matching the
  script's existing pre-flight style; keep away from PR #64's escrow
  constructor region.
- [x] 3. Extend the preflight test suite (4 cases per spec). Repo gotchas:
...
- [x] 4. Gate (solc-serialized: `while pgrep -x solc >/dev/null; do sleep 30; done`
...
- [x] 5. Commit on `chore/deployplanb-seat-invariants`, push, open PR with
