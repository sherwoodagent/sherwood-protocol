...
- [x] 2.2 `SushiLaunchAdapter` singleton: pull quote, unwrap launch-fee ETH from quote WETH, `launchAndBuy(recipient = msg.sender)` so the RESERVE lands on the strategy, then `transferCreator(token, p.feeRecipient)` so the FEE role lands on the vault; zero-balance/zero-role postcondition asserted in tests.
...
- [x] 2.4 Fork tests (4663): full launch with WETH quote; reserve floor enforcement; WOOD quote reverts pre-registration; the creator role is the fee recipient (never the caller) immediately after launch, `distributeFees` pays BOTH legs to the vault and nothing to the strategy.
...
- [x] 3.3 Clone verbs gated by where value goes: permissionless `finalize` (graduate/bond by phase) and `collectFees` (`flushCreatorQuote`, then sweep the clone's whole quote + launch-token balance to the `feeRecipient` fixed at initialization, never to the caller and never through the strategy); owner-only `abort` passthrough while trade-free, returning recovered supply to the OWNER; `phase` derived from `getLaunch` flags per task 0.1's findings.
- [x] 3.4 ERC-1167 target introspection helper so consumers can verify clone → allowed implementation.
- [x] 3.5 Fork tests (4663): fair-launch shape (zero `quoteIn`), reserve withheld before arm, curve dev-buy via pad `buy(recipient = strategy)`, graduation + bond driving, stock-lane `StalePrice` surfaced unchanged, foreign-caller reverts on every clone verb, and a random caller driving `collectFees` to the same fixed destination whether the owner is live or settled.
...
- [x] 4.4 `_settle`: `windowEnd` gate that is revert-proof whenever `block.timestamp >= executedAt + strategyDuration`; no venue call at all (the fee recipient is fixed at launch, so there is nothing to hand over and nothing to collect into strategy custody); quote→asset conversion, push; no fund-token sale into its own pool (test asserts no pool interaction in settle).
