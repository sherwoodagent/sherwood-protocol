...
Gate 3 SHALL be skipped entirely when `coverageFreezer` is the zero address: with no freezer wired, no challenge can freeze coverage and no convictor can reach `forfeitBond`, so no conviction is reachable and gates 1-2 remain a sufficient hold — an unwired or rotated-away freezer MUST NOT strand honest proposers' bonds. For an unchallenged proposal `challengeableUntil[reviewKey]` is zero (an untouched key), so gate 3 reduces to the game's ordinary window and reclaim proceeds on the same schedule as before whenever the game's window does not exceed the ledger's (which the game's own setters enforce). A non-zero freezer whose views revert, cannot be decoded, or ANSWER zero for `challengeWindow` SHALL fail closed: a genuine `ChallengeGame` never answers zero (its setter rejects it), so a zero answer can only come from a non-genuine freezer, and accepting it would place the deadline at `executedAt` — already elapsed — releasing the bond while a conviction remains reachable. This SHALL be scoped to `challengeWindow` alone; `challengeableUntil[reviewKey] == 0` is the ordinary answer for an unchallenged proposal and MUST keep passing. A bond therefore cannot be reclaimed while it could still be forfeited: the filing deadline — including any `challengeableUntil` extension re-armed by an `Inconclusive` unwind — and any live freeze both block reclaim, and no owner-side re-point can lift that hold.
...
#### Scenario: Zero recorded ledger fails closed regardless of the live slot
- **WHEN** a proposal with a non-zero bond and a zero recorded ledger reaches `reclaimProposerBond` in a terminal state after execution, and the governor's live exposure-ledger slot points at a permissive ledger (no `coverageFreezer`, collapsed window)
- **THEN** the call reverts `ExposureLedgerUnset` and releases nothing — the live slot is never consulted, so the re-point cannot open the gates by way of an unrecorded ledger; the same revert follows when the live slot is itself zero

#### Scenario: Freezer answering a zero challenge window fails closed
- **WHEN** the recorded ledger's `coverageFreezer` is non-zero and answers zero for `challengeWindow`, and an executed proposal's other gates have all elapsed
- **THEN** `reclaimProposerBond` reverts `ChallengeWindowOpen` rather than treating the deadline as already past; rotating `coverageFreezer` on the recorded ledger (to a working game, or to zero) restores the ordinary path, so nothing is permanently stranded
