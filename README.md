...
| `src/FeeConstants.sol` | Single source of truth for the protocol-wide fee ceilings and the bps denominator: `MAX_PERFORMANCE_FEE_BPS` (25%, the outer bound governance can ever reach), `DEFAULT_MAX_PERFORMANCE_FEE_BPS` (20%, the advertised headline a new vault starts with), `DEFAULT_AGENT_FEE_BPS` (5%). |
...
- **Two-number fees** — depositors see two charges; everyone who earns is paid out of
  ...
  |---|---|---|---|---|---|
  | Management | fund assets × time deployed | `vault.managementFeeBps()`, seeded from `factory.managementFeeBps` | `MAX_MANAGEMENT_FEE_BPS` = 3%/yr (testnet deploys 0.5%) | **every** settlement — profit, flat or loss | agent 70 / protocol 20 / guardian 10 |
  | Performance | value above the fund's previous **peak** price per share | `vault.agentFeeBps()` (default 5%), clamped at propose to the governor's `maxPerformanceFeeBps` | per-vault cap defaults to 20%; hard protocol ceiling 25% | profitable settlements only | agent 60 / protocol 15 / guardian 15 / owner 10 |
