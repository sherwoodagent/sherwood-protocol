...
- **Two-number fees** — depositors see two charges; everyone who earns is paid out of
  ...
  | Performance | value above the fund's previous **peak** price per share | `vault.agentFeeBps()` (default 20%), clamped at propose to the governor's `maxPerformanceFeeBps` | per-vault cap defaults to 20%; hard protocol ceiling 25% | profitable settlements only | agent 60 / protocol 15 / guardian 15 / owner 10 |
