...
/// @notice Vnet ops: clone the unlevered CL template and open an unlevered
///         NVDA/USDG 0.05% LP proposal for the guardian fleet.
/// @dev    Not a deployment script: every address is the live vnet book and the
///         run is expected once per proposal. Band follows the datanet momentum
///         pod (±3% ≈ ±300 ticks).
contract ProposeUnleveredNvdaVnet is Script {
    ...
    int24 constant HALF_WIDTH_TICKS = 300;
    ...
    function run() external {
        ...
        // Derive both edges from ONE snapped center. Snapping each edge outward
        // widens the band past `2 * halfWidthTicks` for an off-grid spot, and
        // `_requireValidRerangePolicy` rejects that.
        ...
        // A clone whose proposal was blocked is still proposable: its
        // Pending->Executed ratchet only flips at execute.
