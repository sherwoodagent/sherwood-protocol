...
/// @title  DeployWood
/// @notice Fork / beta WOOD bootstrap. Deploys the in-repo WOOD FIXTURE
///         (explicitly non-production — see WOOD natspec), which mints its full
///         supply to the deployer for guardian / owner staking + registry
///         seeding, and persists WOOD_TOKEN so Deploy.s.sol can wire the full
///         guardian stack against it.
///
...
///
/// @dev    env: ALLOW_FIXTURE_WOOD (required, "true"), WOOD_MINT (default 100M).
///
...
contract DeployWood is ScriptBase {
    ...
    function run() external {
        ...
        WOOD wood = new WOOD("Wood Token", "WOOD", mintAmount);
        vm.stopBroadcast();
