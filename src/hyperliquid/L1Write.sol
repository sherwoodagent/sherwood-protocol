...
library L1Write {
    ...
    // NOTE: There is no on-chain CoreWriter action for setting leverage.
    // Hyperliquid's spec defines actions 1-15 only. Leverage must be set via
    // the off-chain exchange API (`updateLeverage`) before opening positions.
