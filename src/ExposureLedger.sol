...
contract ExposureLedger is Ownable2Step, IExposureLedger {
    ...
    ///           2. RESIDUAL CRASH LAG of up to `window + maxDelay`, inherent to
    ///              averaging behind a staleness bound.
    ///
    ...
    ///         that — otherwise one routine missed-quorum failure anywhere in a
    ...
    ///          single routine missed-quorum failure anywhere in its history
    ///          blocks retirement of the entire book;
    ...
    function unfreezeCoverage(address governor, uint256 proposalId) external onlyFreezer {
        ...
        if (_frozen[key]) {
            ...
            for (uint256 i = 0; i < listed.length; i++) {
                ...
                _frozenCommitments[g]--;
                _rebucket(key, g, _restingEpochOf(key, g));
            }
        }
    ...
    ///      the challenge that just failed on a missed quorum. Without this, both of
    ...
    function _rebucket(bytes32 key, address guardian, uint256 target) internal {
        ...
        uint256 wood = r.wood;
        _buckets[guardian][from] -= wood;
        _buckets[guardian][target] += wood;
        // `target` is an epoch index, bounded like `epoch` at booking.
        // forge-lint: disable-next-line(unsafe-typecast)
        r.epoch = uint64(target);
