...
contract ExposureLedger is Ownable2Step, IExposureLedger {
    ...
    /// @inheritdoc IExposureLedger
    /// @dev Converts what a guardian UNDERWROTE into the bps `slashToEscrow`
    ///      speaks. NOT an exact bridge to what a verdict can take: this reads
    ///      the LIVE bond while `_slashOne` slashes `min(checkpointAt(openedAt),
    ...
    ///      in bps of stake; the ledger books liability in USD. Dividing one USD quantity by the
