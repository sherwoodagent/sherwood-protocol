...
- `Layout` struct — `src/strategies/LeveragedAerodromeCLStrategy.sol:171` vs `src/strategies/LeveragedAeroManager.sol:103` (also `RedeemRequest` at `:162` / `:94`).
- `STORAGE_SLOT = 0x405ae0b1…844900` — `src/strategies/LeveragedAerodromeCLStrategy.sol:219` vs `src/strategies/LeveragedAeroManager.sol:151` (identical literal, both flagged CORRUPTION-CRITICAL).
- `_layout()` assembly accessor — `:222` vs `:154`.
...
`vault.totalAssets()` reads ~float-only during the (indefinite) proposal — Lane A is OFF (this `kind` is unregistered in the PriceRouter). Integrators must read `strategy.nav()`, never `previewRedeem` on the vault.
...
|---|---|---|---|
| `execute()` | `BaseStrategy.execute` → `_execute` `:514` | vault (batch) | `onlyVault`, state `Pending` |
| `deposit(assets,minShares)` | `:695` | anyone (whitelisted depositor, enforced vault-side) | `nonReentrant`, state `Executed` |
| `deployIdle(amount,minLiq)` | `:717` | proposer | `onlyProposer`, `Executed` |
| `compound(minUsdcOut,minLiq)` | `:736` | proposer | `onlyProposer`, `Executed` |
| `rerange(minLiq0,minLiq1)` | `:766` | proposer | `onlyProposer`, `Executed` |
| `adjustLeverage(targetLtv,minLiq,minOut)` | `:782` | proposer | `onlyProposer`, `Executed`, `target≤maxLtv` |
| `deleverage(minOut)` | `:798` | **anyone** | `nonReentrant`, `Executed` only |
| `redeem(shares,minAssetsOut)` | `:818` | share owner (fast path) | `nonReentrant`, `Executed` |
| `requestRedeem` / `fulfillRedeem` / `cancelRedeem` / `emergencyRedeem` | `:945/:966/:981/:998` | see Roles table | mixed |
| `settle()` | `BaseStrategy.settle` → `_settle` `:525` | vault (batch) | `onlyVault`, state `Executed` |
| `rescueToVault(token)` | `:1072` | proposer OR vault owner | `nonReentrant`, non-position token only |

There is **no `execute`-time flat-book bypass**: `nav()` reads the `tokenId==0` idle-USDC branch pre-deploy/post-settle (`:451`) and the oracle branch while a position is open.
...
- `Layout` groups: valuation config (token/venue/feed addresses + oracle params), venue addresses, risk params (`targetLtvBps`/`maxLtvBps`/`minHealthBps`/`maxSlippageBps`/`usdcCollateralFactorBps`), position state (`tokenId`/`posTickLower`/`posTickUpper`), fee params+state (`hwmPerShare`/`lastFeeAccrualTimestamp`/`protocolFeeOwed`), the appended `aeroUsdFeed`, and the trailing async-redeem queue (`nextRedeemRequestId` + `mapping(id⇒RedeemRequest) redeemRequests`). `src/strategies/LeveragedAerodromeCLStrategy.sol:171-216`.
- New fields are **append-only** (comments mark the L9 `aeroUsdFeed` and the queue as later appends) — required both for the manager byte-identity and because a nested mapping bars struct-return (hence the separate `LayoutView` mirror at `:232`, exposed via `layout()` `:275`).

**Instance creation.** Instances are **ERC-1167 clones**, not UUPS proxies — there is no upgrade path for a live strategy. Deploy: `StrategyFactory.cloneAndInit(template, vault, proposer, data)` → `Clones.clone(template)` → `initialize(vault, proposer, data)` (`src/StrategyFactory.sol:127`). Guards: template must be on the owner allowlist (`approvedTemplate`, `:59`/`:102`), `vault` registered, and `proposer == msg.sender` (`ProposerMustBeSender`, `:133`). `initialize` is one-shot (`BaseStrategy.sol:87`); the template itself is locked in its constructor (`:64`). `_initialize` (`:334`) validates and persists `InitParams` — the audit-relevant checks:

| Check | Site | Enforces |
|---|---|---|
| `usdc == vault.asset()` + 6 decimals | `:360-361` | unit-of-account = vault asset; `SHARES_VIRTUAL_OFFSET=1e6` hardcodes 6dp |
| `maxLtvBps < Moonwell CF` | `:366` | leverage cap below the collateral factor |
| `minHealthBps × maxLtvBps < 1e8` | `:370` | deleverage trigger LTV strictly above `maxLtvBps` (no grief-deleverage band, L4) |
| oracle-param bounds | `:378-382` | `maxDelay∈(0,7d]`, `grace≤1d`, `twapWindow∈(0,1d]`, `calmTicks∈(0,5000]`, `slippage∈(0,1000]` — a misconfig can't disable a guard |
| `aeroUsdFeed.decimals()==8` | `:356` | L9 floor scaling assumption |
| fee ceilings | `:387-388` | perf ≤ 1500 bps (`FeeConstants.MAX_PERFORMANCE_FEE_BPS`), mgmt ≤ 500 bps |
...
| `deposit` | | | | | ✅¹ | `Executed`; vault re-checks whitelist+pause | `:695`, hook `:916` |
| `deployIdle` | | | ✅ | | | `onlyProposer`,`Executed` | `:717` |
| `compound` | | | ✅ | | | `onlyProposer`,`Executed` | `:736` |
| `rerange` | | | ✅ | | | `onlyProposer`,`Executed` | `:766` |
| `adjustLeverage` | | | ✅ | | | `onlyProposer`,`Executed` | `:782` |
| `deleverage` | | | | | ✅ | `Executed` only (health-gated in impl) | `:798` |
| `redeem` (fast) | | | | | ✅¹ | `Executed`,`nonReentrant` | `:818` |
| `requestRedeem` | | | | | ✅¹ | `Executed` | `:945` |
| `fulfillRedeem` | | | ✅ | | | `onlyProposer`,`Executed` | `:966` |
| `cancelRedeem` | | | | | request owner only | any state | `:981` |
| `emergencyRedeem` | | | | | request owner only | `Executed`, after `FULFILL_WINDOW` | `:998` |
| `rescueToVault` | | | ✅ | ✅ | | `NotProposerOrOwner`; non-position token | `:1072` |
| `updateParams` | | | (no-op) | | | `_updateParams` empty | `:1084` |
| `strategyMint(to,shares)` (vault) | | | | | active strategy | `onlyActiveStrategy` + `whenNotPaused` + depositor whitelist | `SyndicateVault.sol:916` |
| `strategyBurn(shares)` (vault) | | | | | active strategy | `onlyActiveStrategy`, **NOT** `whenNotPaused` | `SyndicateVault.sol:931` |

¹ `deposit`/`redeem`/`requestRedeem` are open to any caller, but `deposit`+`strategyMint` re-enforce the vault depositor whitelist (`_requireApprovedDepositor`, `SyndicateVault.sol:917`); a closed vault stays closed through the strategy.
...
**`nav()`** (`src/strategies/LeveragedAerodromeCLStrategy.sol:448`) returns USDC (6dp), **net of `protocolFeeOwed`** (`:461-462`, floored at 0, never reverts on owed>gross):
...
- Active position: `LeveragedAeroValuation.netEquityUsdc` (`:459`), which is fail-closed.
...
**`selfManagesFees() == true`** (`:504`). The governor skips `_distributeFees` **entirely** for this proposal (protocol + guardian + agent + management), so the strategy self-collects. Consequence: the **protocol** fee is collected strategy-side.
- **Management** (streaming) + **performance** (HWM per-share) crystallize by **minting fee-shares** to `feeRecipient` via `strategyMint` (`_crystallizeFees`, `:552`; math in `LeveragedAeroFees.crystallize`, `src/strategies/LeveragedAeroFees.sol:199`). HWM seeds on first cycle (no phantom fee, `:140`); crystallize is on the **pre-action** NAV, before USDC is pulled (phantom-fee fix).
- **Protocol slice** is taken off the **gross gain above HWM first** (`LeveragedAeroFees.sol:153`), accrued as a USDC liability `protocolFeeOwed` (never minted as shares — treasury must receive USDC), and **netted out of `nav()`**. HWM still advances to the gross peak (no double-charge). Discharged where USDC flows: `compound` skim (`:747`), `_settle` (`:533`), and the async redeem skim `_dischargeRedeemSkim` (`:1051`). The fast-path `redeem` takes **no** skim — `nav()` is already net, so pricing at `f×navNet` preserves stayers (a skim would double-charge).
- Crystallize is **best-effort** on user-exit paths (`_crystallizeBestEffort`, `:656`): a fee-mint revert (paused vault / de-whitelisted recipient) or the near-unreachable governor read rolls back **only** the crystallize (HWM + timestamp + owed together) and emits `FeeCrystallizeDeferred`; the exit proceeds. `compound`/`_settle` read the governor **un-try'd** and hard-revert on the same failure (asymmetry is intentional).
...
4. **No exfil** — `invariant_noExfil` (`:137`): the strategy only ever pays `vault()` or a redeeming user. `rescueToVault` target is hardcoded to `vault()` (`:1080`), never caller-supplied, and reverts `CannotRescuePositionToken` for usdc/cbBTC/weth/mTokens/AERO (`:1076`).
5. **`redeem` never over-pays.** Fast path pays `mulDiv(shares, navNet, supply)` rounded **down** (`:836`); `assetsOut==0 ⇒ ZeroAssetsOut` (`:841`) blocks burn-for-zero. Async path pays the gross proportional unwind minus the pro-rata protocol skim, min-out enforced (`:1039-1040`).
6. **`protocolFeeOwed` monotonic until discharged.** Increases only in `_crystallizeFees` (`:582`); decreases only at the three discharge points paying USDC out (`:537`, `:749`, `:1060`). If `protocolFeeRecipient == 0`, discharge is skipped and the liability persists (`nav()` stays net) until a recipient exists.
7. **`emergencyRedeem` deadman** requires `msg.sender == request.owner` and `block.timestamp > requestedAt + FULFILL_WINDOW` (2 days) with `!settled` (`:1001-1003`); `settled` is a single-shot double-spend guard set on fulfill/cancel/emergency.
...
9. **No phantom fee**: crystallize on pre-deposit NAV ⇒ zero perf fee on idle USDC that just landed (`LeveragedAeroFees` seed/ordering `:136-169`; deposit ordering `:695-711`).
10. **Deposit share-inflation guard**: `navNet==0 && supply>0 ⇒ NavUnpriceable` (`:707`); first deposit (`supply==0`) legitimately allowed.
...
- **Donation / inflation on strategy-side mint.** `nav()` excludes vault float (M2) but **counts** strategy-held idle USDC and out-of-position legs (`:455`, valuation `:135-136`); consider direct-donation griefs to the strategy address inflating `nav()` and the share price for a pending depositor. `NavUnpriceable` guards the `navNet==0` inflation case only.
...
- **Governance blast radius (acknowledged).** `ABSOLUTE_MAX_STRATEGY_DURATION` is a global governor `public constant` (3650 days, `src/GovernorParameters.sol:37`); once raised via UUPS upgrade, any vault owner can set a multi-year `maxStrategyDuration`, keeping *that vault's* owner emergency exits dormant. Per-vault duration cap is the deferred mainnet hardening.
...
- **Users hold vault ERC-20 shares** (12 dp for a 6 dp USDC asset). The strategy never issues its own
  ...
  (`src/SyndicateVault.sol:916`, `:931`). Your on-chain balance is `vault.balanceOf(user)`.
...
```
`src/strategies/LeveragedAerodromeCLStrategy.sol:695`
...
(`:360`). Share offset is `1e6` matching the vault's `_decimalsOffset()`.

**Approvals** — `usdc.approve(strategy, assets)` (or Permit2). USDC is pulled with `safeTransferFrom`.

**Preconditions**
- Strategy `state() == Executed` else `NotExecuted` (`:696`).
...
- Vault gates re-checked inside `strategyMint` (`:916`): `whenNotPaused` (else `EnforcedPause`) and the
...
(`:708`). `navNet` = pre-deposit `nav()` minus the fresh protocol-fee slice the crystallize accrued; fees
...
```solidity
function requestRedeem(uint256 shares, uint256 minAssetsOut) external nonReentrant returns (uint256 id);      // :945
function fulfillRedeem(uint256 id) external onlyProposer nonReentrant;                                          // :966
function cancelRedeem(uint256 id) external nonReentrant;                                                        // :981
```
...
```solidity
function emergencyRedeem(uint256 id, uint256 minAssetsOut) external nonReentrant returns (uint256 assetsOut); // :998
```
...
`:89`). Covers the only truly stuck case (oracle down **and** backend dead); a live backend resolves via
...
|---|---|---|---|
| NAV | `nav() → uint256` (`:448`) | USDC 6 dp | Net of `protocolFeeOwed`. Reverts (fail-closed) when oracle down / calm-gate breached. Wrap in try/catch off-chain. Refresh per block / on-demand. |
...
| Redeem quote | `previewRedeem(shares) → (assetsOut, fastOk)` (`:889`) | USDC 6 dp, bool | Mirrors executed `redeem` incl. pending fee. `(0,false)` when unpriceable. Apply small slippage tolerance for `minAssetsOut` (mgmt fee accrues with `dt`). |
| LTV / health basis | `previewCollateralDebt() → (collateralUsdc, debtUsdc)` (`:930`) | USDC 6 dp | **Self-only** (`OnlySelf` guard) — used internally by `previewRedeem`; NOT frontend-callable. To show LTV/health, read Moonwell markets directly, or use `previewRedeem(shares).fastOk` as the routing signal. Risk caps live in `layout().maxLtvBps` / `.minHealthBps`. |
| Pending request | `redeemRequest(id) → RedeemRequest{owner, shares, minAssetsOut, requestedAt, settled}` (`:316`) | — | `requestedAt` (uint40) + 2 days = emergency-eligible time. `nextRedeemRequestId` = next id via `layout()`. |
| Fee params / risk / config | `layout() → LayoutView` (`:275`) | mixed | `managementFeeBps`, `performanceFeeBps` (bps), `feeRecipient`, `hwmPerShare` (1e18 WAD), `protocolFeeOwed` (USDC 6 dp), `targetLtvBps`/`maxLtvBps`/`minHealthBps`/`maxSlippageBps` (bps), token/venue/feed addresses. One call. |
...
| `RedeemEmergency(id, owner, assetsOut)` | strategy `:95` | id, owner | `emergencyRedeem` | Yes — deadman exit paid. |
...
- **Burn works during pause, deposit does not.** `strategyMint` is `whenNotPaused` (deposits + fee mints
  ...
  incident (`src/SyndicateVault.sol:916`, `:931`). Governance settlement still respects pause.
...
- **Fee caps** (init-enforced): management ≤ 500 bps/yr, performance ≤ 1500 bps
  (`FeeConstants.MAX_PERFORMANCE_FEE_BPS`). Protocol fee is read live from the governor and accrues as the
  `protocolFeeOwed` USDC liability netted out of `nav()`.
