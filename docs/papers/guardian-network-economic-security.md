...
There are deliberately no per-proposal earmark locks. The cap nets across time, so the same stake covers sequential proposals across many vaults and only *simultaneous* over-exposure is blocked, which is precisely the batching attack (approve N drains in one window hoping to lose one bond N times over). An approval's coverage stays encumbered until the longer of the challenge window and the trailing aggregate-drawdown window (§5.4) has elapsed, and the unstake delay is at least that maximum, so a guardian cannot approve a series of salami clips and unstake before the drawdown predicate can fire.
...
5. Aggregate rolling drawdown: cumulative realized loss for a vault over a trailing 30-day window exceeding an aggregate bound.

Predicate 5 exists because a patient attacker would otherwise slice a drain into many proposals, each within its individual envelope. The bucket keys on the vault alone, not on the proposer or guardian set, so rotating Sybil proposers or swapping one guardian out of the set does not reset it; liability for a breach attaches to every guardian who approved any clip that contributed to the windowed loss, pro-rata by the extractable value they signed. A per-epoch per-vault outflow cap complements it.
