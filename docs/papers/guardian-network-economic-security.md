...
**Propose and vote.** An agent submits a proposal with its execution calls and settlement calls fixed up front, together with its risk envelope (§5.1). A proposal that moves extractable value also requires the proposer to post a bond scaled to its max-extractable value, first-loss ahead of any approver stake (§5.8). Depositors vote during the voting window. Passage is optimistic only for proposals that move no extractable value (a pure settle, or a rebalance with zero net outflow): when voting ends, such a proposal is approved unless AGAINST votes reached the veto threshold, a fraction of the shares that could vote when the proposal opened (supply minus the withdrawal queue), and silence passes. A coverage-consuming proposal cannot pass on silence; it must clear the approve quorum below.
...
(C3)  a challenge carried by the guardian vote slashes the proposer bond
      first, then each approver's attributed exposure at 100%
                                                       (challenge game, §5.4;
                                                        guardian vote, §5.5)
(C4)  slashed approver stake is burned in full; the forfeited proposer bond is
      burned save a prosecutor fee to the challenger, hard-capped
      at 20% of the bond                               (burn destination, §5.9;
                                                       prosecutor fee, §5.4)
```

A coverage-consuming proposal cannot execute at all without approvers whose bonds cover its extractable value (the approve quorum, §5.3), so every unit of E_i is attributed to approver coverage that cannot be withdrawn while the window is open. When the drains execute and are successfully challenged, the slashed stake is at least the attributed exposure. The only payment the mechanism ever makes on a conviction is the prosecutor fee, a fraction φ ≤ φ_max = 20% of the proposer bond B_p paid to the challenger (§5.4); grant the coalition the worst case, in which it files the challenge against itself so that fee round-trips. With the tier-2 value k = 1:

```
Π  =  Σ_i V_i  −  B_p  −  (slashed approver stake)  +  (recovered prosecutor fee)
                                                        −  bribes  −  gas
   ≤  Σ_i E_i  −  B_p  −  Σ_i E_i  +  φ_max · B_p  −  bribes  −  gas   (C1–C4, k = 1)
   =  −((1 − φ_max) · B_p  +  bribes  +  gas)  <  0          since φ_max < 100%
```

The slashed stake contributes nothing to recoupment, by construction rather than by assumption: every wei of it is burned (C4), so there is no payout to claim, no snapshot to have pre-bought, and no payee to become. An earlier revision of this design achieved recoupment = 0 differently, routing slash proceeds into an escrow of non-transferable compensation claims fixed to pre-drain holders of record; burning strengthens the property from an access-control guarantee into a nonexistence one, at the stated cost that harmed depositors are not made whole onchain (§5.9). The one channel that does flow back — the self-filed prosecutor fee — is bounded at φ_max of a bond forfeited in full, a strict loss at any fee rate below 100%, and the cap is enforced by the escrow that actually moves the funds, not merely by the game that requests the payment.

The shape of this argument is the cost-of-corruption inequality familiar from optimistic oracles and restaking: the system stays honest when the cost of corrupting it exceeds the profit from doing so [7, 8], with the profit side bounded by an explicit accounting of extractable value rather than assumed [7]. The bound rests on three assumptions, each discharged by a mechanism rather than by trust: that realized extraction cannot exceed the certified bound (tier guards and custody metering, §5.1–5.2); that a valid challenge is filed within the window (the funded watchtower and first-detector bounty, §5.4); and that a quorum of guardians outside the accused cohort votes to convict on a true predicate (the guardian vote on a pre-filing snapshot, §5.5). It does *not* assume guardians are honest, attentive, or even distinct from the proposer, who may control every approving key. The bound holds because the value that can leave is mechanically capped (R2), the stake that answers for it is mechanically at risk (R1), and the confiscated value cannot leak back to the attacker, because it is destroyed rather than paid out (C4). Liability attaches only to the proposer who bonded the proposal and the guardians who approved it; others are untouched.

## 5. The mechanism

The components divide along R1 and R2. Risk envelopes, adapter tiers, and the aggregate exposure cap (§5.1–5.3) bound what a proposal can extract and require covering signers before it executes. The challenge game and the guardian vote that decides it (§5.4–5.5) make executed-outcome liability enforceable, and the burn destination (§5.9) makes the confiscated value uncollectable by anyone, the attacker included. The remaining subsections cover the supply side: adapter listing (§5.6), delegated stake (§5.7), and the proposer bond (§5.8).
...
A filing opens a guardian vote and is decided by it (§5.5). A filed challenge freezes only the accused guardians' coverage attributed to that specific proposal, never their whole stake, and the challenger's bond scales with the exposure it freezes; both properties exist to keep challenges from becoming a griefing weapon against honest guardians. A challenge that fails burns a fifth of the challenger's bond and returns the rest — to the challenger, not to the accused, who are never a payee on any path. A coalition therefore cannot profit by filing against itself: the slashed stake is burned (§5.9), the only thing a challenger is ever paid on a conviction is a capped fraction of the proposer's own forfeited bond, so a self-filed challenge recovers at most a fifth of a bond lost in full, and a self-filed challenge that is simply never convicted is a fifth of the challenger's bond destroyed for nothing. The recoupment channel is closed at the payout, not at the challenge.

Whether anyone files is itself an economic question. The slashed guardian stake S pays nobody — every wei of it is burned (§5.9). What a conviction pays the challenger is a *prosecutor fee*: a fraction φ of the proposer's forfeited bond B_p, pinned at filing time (20% at launch, which is also the hard cap, and the cap is re-checked by the escrow that actually moves the funds rather than trusted from the game). A challenger posts bond b_c, pays forensic cost c_f to build the case, and carries the guardian vote with probability p_win:

```
EV(challenge) = p_win · φ · B_p  −  (1 − p_win) · b_c  −  c_f
```

which is positive exactly when

```
p_win  >  (b_c + c_f) / (φ · B_p + b_c)
```

The funding source is the point, not a detail. The slash is a pot a prosecutor *can* fund for itself, by staking and approving the very proposal it is about to accuse; a slash-funded fee therefore needs a role predicate to price that sybil, and the predicate an earlier design used was controlled by the accused, who could zero the fee for free by funding their defence from an unrelated wallet. The proposer's bond has neither problem. It is the one pot in the protocol a prosecutor cannot fund for itself: to self-deal you must *be* the proposer, and then you are paying yourself a fraction of a bond you forfeit in full — a net loss at any fee rate below 100%. That makes the fee sybil-proof by construction, with no role predicate to game and no cap to tune for the sybil's sake.

For a clear-cut violation p_win is high and the condition is easy to satisfy — though p_win is the probability that a quorum of *other* guardians actively votes to convict, not the probability that a forensic question has a correct answer, and an inattentive cohort drives it down independently of the merits. But φ must be kept small — every basis point paid to the challenger is a basis point of the attacker's forfeiture un-burned, and in the self-filed worst case a basis point the coalition recovers (§4) — and c_f is real work, so for marginal cases the organic reward p_win · φ · B_p can undershoot c_f, and a purely permissionless game would leave them unfiled. This is why the watchtower is funded protocol infrastructure, not an assumption of altruism — the same conclusion reached by watchtower designs in payment channels [13] and by formal treatments of challenger incentives in optimistic protocols, which show that relying on an unpaid honest challenger is not a stable equilibrium [3, 5]. Sherwood's forensic agents run the predicate monitor as a paid role, and a first-detector bounty sized to cover c_f keeps the door open for independent watchers. A healthy game shows a low rate of filings, some failing; total silence is itself a dashboard alarm.

### 5.5 The guardian vote

A filing does not adjudicate itself. It opens a vote of the staked guardian set, and that vote is the only thing that can convict.

**The electorate is pinned at filing, and the accused are not in it.** At the moment a challenge is filed, the mechanism reads total staked WOOD one second earlier and subtracts the stake of every approver the filing accuses. That difference — the *votable stake* — is the denominator for the rest of the challenge's life. Reading one second back matters: a stake checkpoint is keyed on the second it changes, so measuring the filing instant itself would let WOOD planted in that very block sit in the numerator while the denominator missed it. Subtracting the accused cohort does two things at once. Nobody who underwrote the challenged proposal may cast a ballot, and their weight does not inflate the bar the remaining cohort must clear, so a guardian cannot immunize itself by holding a stake large enough that the rest of the network could never out-vote it. If subtraction leaves nothing — the accused cohort *is* the whole staked set — the filing is refused outright rather than accepting a bond it could only destroy.

**Conviction requires an affirmative quorum.** Each eligible guardian votes convict or acquit once, weighted by its own stake at that same pinned instant, with no changes and no ballots after the window closes. The challenge settles when convict weight reaches a fixed fraction of the votable stake (30% at launch, bounded to [10%, 100%] and pinned onto the challenge at filing so no later parameter change can re-rate a live case). Because there is no un-vote, a reached quorum is already final and settles on the spot. Abstention and acquittal are arithmetically identical — both leave the numerator alone — which means the burden sits entirely on the accusing side. A challenge carries because guardians actively voted to convict, never because nobody defended.

**Missing quorum costs the challenger.** When the window closes short of the bar, the challenge fails: a fifth of the challenger's bond is destroyed and the remainder returns to the challenger. Silence and acquittal fail the challenge identically in money terms and differ only in what they say about the proposal. A window that closed with no ballot at all adjudicated nothing, so the proposal's challenge window re-arms and a better-prepared filer may try again — once per proposal, because an unbounded re-arm would let a filer cycling addresses pin an honest cohort's coverage indefinitely. Any acquit weight at all means the cohort looked and decided, and spends the window.

**Constraint C4 holds by construction on every path.** No transfer this mechanism makes can reach an approver or the proposer. A conviction burns the approvers' locked stake inside the staking contract, which has no payee argument to name; it forfeits the proposer's bond, of which at most a fifth reaches the challenger as the prosecutor fee and the rest is destroyed; and it returns the challenger's own bond less a settle burn. A failure burns a fifth of the challenger's bond and returns the rest to the challenger. Every burn goes to an address nothing can spend from. The accused are therefore never a payee, which is what makes a self-filed challenge a pure loss in both directions: convict yourself and you pay a bond forfeited in full to recover a fifth of it; fail against yourself and you have destroyed a fifth of the challenger bond to freeze coverage you also own.

**What this mechanism does not do.** It does not adjudicate forensics. There is no panel, no appeal, no bad-faith track, and no on-chain verification of the predicate a filing cites — the predicate is a label carried in the event for watchtowers and indexers to route on. What the vote measures is whether a quorum of *disinterested staked guardians*, having read the evidence the filing points at, is willing to put its name on a conviction. The bound therefore rests on an attentiveness assumption rather than a forensic-adjudication one, and that is not a free trade: an inattentive cohort acquits by doing nothing, and the §5.4 filing condition must be read with p_win as the probability of reaching quorum rather than the probability of being right. The defence against a cohort bought rather than merely idle is the same pinned-and-excluded snapshot described above — drain proceeds cannot buy an acquittal because acquittal is the default, and cannot buy a conviction of someone else because the stake that votes is measured before the filing and the accused are removed from it. Pre-accumulation remains mitigated rather than closed: an attacker who bought WOOD well before the exploit holds votable stake like anyone else, and its remedy is the deterrence that the conviction it would need to suppress is the one its own approvers already paid for in locked coverage. This is an accepted residual (§7).
...
The proposer is the actual attacker, and until it posts capital of its own the coalition arithmetic asks approvers to backstop a party with nothing at stake. A coverage-consuming proposal therefore requires the proposer to post a bond scaled to its max-extractable value, a fraction tuned so honest proposers are not priced out but a rug forfeits meaningfully. On a passed challenge the proposer bond is forfeited *before* any approver stake is touched — the prosecutor fee carved out for the challenger, the remainder burned (§5.4, §5.9) — so the attacker's own capital is first-loss and approver coverage is the second layer, not the first. The emergency-settle owner bond, which previously sat at a flat amount with no relation to what a settle could extract, scales the same way.

### 5.9 Slash destination: burn (supersedes the compensation escrow)

Where slash proceeds go is not a detail, and it is the one place the shipped mechanism deliberately departs from earlier revisions of this paper. Confiscated value is **burned**: a verdict slash destroys every wei taken from the approvers, and the forfeited proposer bond is destroyed save the challenger's prosecutor fee (§5.4). Depositors are not compensated. Guardians are a punitively bonded quality filter, not an insurance pool.

Earlier revisions routed slash proceeds to a compensation escrow. Paying a slash into the vault's live balance was never an option — the vault is an ERC-4626 share token with freely transferable shares, so a coalition could drain it, let honest holders redeem at the depressed NAV, buy their shares cheaply, and collect a large fraction of the slash when it landed. The escrow answered that with a snapshot: non-transferable, per-address compensation claims minted pro-rata to shares held at the pre-drain block. That design was implemented and then removed, for two reasons that survive in the contracts' own documentation.

First, every reachable payee is a round trip to the attacker. Paying the challenger fails because addresses are free: a proposer can file against its own drain and refund itself the value it was supposed to lose. Paying the vault's LPs against a pre-drain snapshot is the closest to the ideal, but the proposer may itself hold shares in that snapshot — it could have bought in before proposing — and recovers its pro-rata slice. Paying a treasury pays whoever governs, which the attacker may be or may lobby. The snapshot escrow narrowed the recoupment channel; destruction is the only sink with *no beneficiary to be*, and closes it by nonexistence rather than by access control.

Second, a compensatory sink weakens the slash that feeds it. Compensation cannot pay a victim more than their loss without creating a windfall, so the escrow bounded the slash at 1× damages: each approver's rate was derived from its pro-rata share of required coverage, which left an approver of a value-V attack roughly break-even on conviction. Burning has no counterparty and therefore no windfall constraint — the slash rate is the severity ceiling for every committed approver, and deterrence is no longer capped by what compensation arithmetic can justify. (Deriving the rate from coverage also priced the slash through an asset feed, making a conviction unpriceable exactly during the market stress a drain happens in; the burn rate reads no feed.)

The cost is stated plainly: harmed depositors are not made whole onchain, a deliberate deviation from the compensation-first prescription in the restaking literature (§8). What §4 buys with that cost is a recoupment term that is zero by construction — no payout to claim, no snapshot to have pre-bought, no payee to become.

> **Decision note (2026-08-02, issue #65).** Burn-as-destination was adopted
> in the code change that removed the escrow, and it is the *reversible*
> choice: burning is strictly more punitive than any routing, so adopting a
> compensation or premium route later is a pure improvement for its
> beneficiaries with nothing to unwind — no accounting to migrate, no funds
> to recover, no live claim to honour. Routing a slice of forfeitures to a
> guardian-premium line is deliberately deferred until the guardian ROE
> programme it would fund exists and is sized (issue #70); until then the
> prosecutor-fee rate (§5.4) is the tuning knob available without any code
> change, up to its 20% cap.
...
The rollout order is deliberate: bound what can move, and guarantee a bonded signer, before building the machinery that imposes liability after the fact, because an order that front-loads the novel adjudication while deferring the machinery that bounds loss is backwards for risk. Step one adds no new trust and bounds loss on its own: risk envelopes and per-proposal outflow metering, adapter tiering with its runtime guards, the submitter bond escrow, the dollar-denominated exposure cap and covered-TVL cap, the explicit approve quorum, and the risk-scaled proposer bond. This step alone hard-caps extractable value and guarantees a covering signer; it degrades safely, since a value-moving proposal without covering approvals simply does not execute, with no adjudication yet. Step two adds retroactive liability: the verdict-driven slash entry point with its burn-only destination (§5.9), the challenge game with all five predicates and its bond-funded prosecutor fee (§5.4), watchtower funding, and the coverage-weighted approver premium. Step three adds adjudication: the guardian vote on the challenge, its pinned pre-filing snapshot with the accused cohort excluded, and the convict quorum that carries it (§5.5).
...
**Cost-of-corruption accounting.** Sizing security to what an attacker can steal, rather than to notional TVL, follows the optimistic-oracle tradition: UMA's data verification mechanism is built on the inequality cost-of-corruption > profit-from-corruption, with registered contracts reporting the value at risk [7] — the direct ancestor of our max-extractable coverage. EigenLayer's whitepaper generalizes the accounting to restaked security and names the failure mode we guard against with the aggregate exposure cap: the same stake implicitly backing multiple concurrent claims [8]. STAKESURE argues slashed funds should compensate harmed users rather than merely burn, and formalizes the resulting notion of strong cryptoeconomic safety [9]; Sherwood deliberately deviates from that prescription. An earlier revision implemented it with a snapshot-based compensation escrow, but every reachable payee is a potential round trip to the attacker — addresses are free, so the challenger may be the proposer, and the proposer may hold LP shares in any compensated snapshot — so slash proceeds burn instead (§5.9), trading STAKESURE's made-whole guarantee for a recoupment term that is zero by construction. Robust-restaking analyses study when networks of shared stake remain secure as coverage overlaps grow [10], the regime our per-guardian cap deliberately avoids entering.
...
**Token votes as adjudicators.** The shape of the guardian vote answers a documented literature: plutocratic token voting is capturable and bribable [14, 15, 16], including via onchain vote-buying and dark-DAO constructions [17]. Sherwood narrows the franchise rather than layering appeals over it — only staked guardians vote, the accused cohort is struck from both the ballot and the denominator, and the snapshot predates the filing — so the bribable quantity is stake that was already committed and already slashable. Kleros remains the reference for what a full adjudication system costs to build [6]; this mechanism deliberately buys less.
...
That is the standard the guardian network is built to. The review window filters proposals before execution, with a deterministic penalty for approving what your peers block. The economic-security layer binds the path that review windows alone cannot reach, through three mechanical facts: nothing leaves a vault beyond its certified extractable value, that value is never larger than the dollar bond that answers for it, and the confiscated stake is destroyed rather than routed anywhere it could leak back to whoever drained it. The proposer's own capital is first-loss, and the guardians who approved a drain pay next, at 1:1, after the fact.
