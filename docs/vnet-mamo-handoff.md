...
`State.Executed` with NAV ≈ $60k. Deployed 2026-07-20 from `sherwood-protocol` `main`
(cb8bca0) on branch `vnet/mamo-leveraged-aero`. The full LeveragedAero fork suite
(**62 tests**) ran green against this vnet before the broadcast campaign.
...
| **LeveragedAerodromeCLStrategy clone** (live) | `0x168ac730AB0DA6FCDE8aA26e33eac4aE6c8CfB4B` |
...
| LEVERAGED_AERO_CL_TEMPLATE | `0x1a4b1cd88862d408257751241cE7124D3FcE34ED` |
...
- `strategy.state() == 1` (Executed); proposal 2 state == 6 (Executed, indefinite: 3650-day duration)
- `strategy.nav() == 59_999_951_255` (≈ $60k USDC, 6dp)
...
4. **Governor `VoteType` is `{For=0, Against=1, Abstain=2}`** (not OZ order). Proposal 1 on this
   vnet is a `Rejected` artifact of that (accidental full-weight Against vote); proposal 2 is the
