...
- **Double clamp:** the fee is clamped against the governor cap at **propose** time
  (snapshotted onto the proposal, `src/SyndicateGovernor.sol`) and re-clamped at
  **settle** against the live cap (`src/SyndicateGovernor.sol`) so a later cap
...
- **High-water mark:** the fee applies only to assets above the highest price per
  share ever charged at (`aboveHighWaterMark()`, `src/SyndicateVault.sol`). The
  mark ratchets monotonically at settlement (`ratchetHighWaterMark`,
  `src/SyndicateVault.sol`) — losses leave it in place, so recovery back to the
...
Seeded in the constructor (`src/ProtocolConfig.sol`). Sum must equal 10 000 bps
(`src/ProtocolConfig.sol`). The vault-owner leg exists only here, not on the
...
- `_payFee` wraps the transfer in `try/catch`: a failing recipient (e.g. blacklisted)
  ...
  (`src/SyndicateGovernor.sol`). Anyone can later push it out via the
  permissionless `claimUnclaimedFees(vault, token)` (`src/SyndicateGovernor.sol`).
...
(`_distributeAgentFee`, `src/SyndicateGovernor.sol`):
...
|---|---|---|
| Vault creation fee | optional absolute ERC-20 amount pulled at `createSyndicate`; default **0** (free) | no bounds; owner-set (`SyndicateFactory.setCreationFee`) |
...
|---|---|---|---|---|---|
| `managementFeeBps` | bps/yr | 200 | 0 | 300 | `SyndicateFactory.initialize`, `setManagementFeeBps` |
| `agentFeeBps` | bps | 2 000 | 0 | 2 500 | `SyndicateVault.setAgentFeeBps` |
| `maxPerformanceFeeBps` (per-vault cap) | bps | 2 000 | 0 | 2 500 | `GovernorParameters.setMaxPerformanceFeeBps` |
| Mgmt split legs | bps | 6000/2000/2000 | 0/leg | sum == 10 000 | `ProtocolConfig.setMgmtSplit` |
| Perf split legs | bps | 5000/1500/2500/1000 | 0/leg | sum == 10 000 | `ProtocolConfig.setPerfSplit` |
| Co-proposer `splitBps` | bps | per-proposal | 100 | 9 000 total | `SyndicateGovernor._validateCoProposers` |
| `creationFee` | absolute | 0 | — | unbounded | `SyndicateFactory.setCreationFee` |
| `minBufferBps` | bps | 0 | 0 | 5 000 | `SyndicateVault.setMinBufferBps` |
