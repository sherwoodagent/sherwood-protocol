...
Status: **the ATTACK half is implemented (PR #243). The FAIRNESS half is not.**
...
- **(b) the underpayment** — a queued redeemer is paid at a stamp that excludes
  ...
  gap: no attacker, no loss to the protocol, a transfer between LPs. **OPEN.**
...
### Invariants the fuzz suite must assert
...
1. **Queue-side round record** (queue-only, no cross-contract calls): per-pid
   cohort share count frozen at `stampSettlement`, plus per-request claimed
   tracking. Unit-testable by pranking as the vault.
2. **Split at arrival**: `collectResidue` sends the cohort's `q` share to the
   queue and leaves the rest in the vault.
3. **`claimRemainder(requestId)`**: pays pro-rata from what the queue holds for
   that pid. Independent of floor-claim order; a zero balance is a no-op, never a
   revert.
4. **Fuzz/invariant suite** for the four invariants above, plus the zero-cohort
   case and interleaved permissionless claims.
5. **Docs**: update this file and close #233.
