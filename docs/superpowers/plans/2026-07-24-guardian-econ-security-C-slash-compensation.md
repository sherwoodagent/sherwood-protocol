...
3. **`escrow` is owner-set state, not a call parameter.** `StakedWood` gained `compensationEscrow` (slot 46, gap `8 → 7`) + `setCompensationEscrow`. **`slashToEscrow`'s arity as of this plan is `(bytes32 caseKey, uint256 openedAt, address[] approvers, uint256 slashBps, address vault, uint256 snapshotTimestamp)`** — the Task 4 snippets above show the older 7-arg form. Uses `forceApprove` and zeroes the allowance after `openCase`. **STALE as of `2026-07-29-court-incentives-design.md` §2:** Task 1 of that plan appended `(address bountyTo, uint256 bountyBps)`, so the real arity today is eight parameters, not six — see that spec for the conviction-bounty channel and its `MAX_CONVICTION_BOUNTY_BPS` cap.
